...
def display():
    ...
    if animTime % 2 >= 1.0:
        animTime = 1 - (animTime % 1.0)
    else:
        animTime %= 1.0
    R3 = R1*Rotation.from_rotvec(animTime*delta)

    cow2wld = matrix4(R3.as_matrix(), getTranslation(cow2wld1) -
                      (getTranslation(cow2wld1) - getTranslation(cow2wld2)) * animTime)
...
def initialize(window):
    ...
    cow2wld1 = glGetDoublev(GL_MODELVIEW_MATRIX).T
    ...
    glRotated(-90, 1, 0, 0)
    # convert column-major to row-major
    cow2wld2 = glGetDoublev(GL_MODELVIEW_MATRIX).T
