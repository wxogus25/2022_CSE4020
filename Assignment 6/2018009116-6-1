...
def render():
    ...
    Rzx = np.array([[1,0,0],
                   [0, np.cos(ZX), -np.sin(ZX)],
                   [0, np.sin(ZX), np.cos(ZX)]])

    Rzxz = np.array([[np.cos(ZXZ), -np.sin(ZXZ), 0],
                   [np.sin(ZXZ), np.cos(ZXZ), 0],
                   [0,0,1]])
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2018009116-6-1', None,None)
    if not window:
        glfw.terminate()
        return
