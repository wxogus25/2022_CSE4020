...
def display():
    global cameraIndex, cow2wld, progress, animStartTime
    ...
    if 0 <= progress < 5:
        for x in range(progress + 1):
            drawCow(sixCow[x], 0)
    elif progress == 5:
        A = np.array([2, -2, 1, 1, -3, 3, -2, -1, 0, 0, 1, 0, 1, 0, 0, 0])
        A.shape = (4, 4)
        B = np.array([0, 1, 0, 0, 0, 0, 1, 0, -.5, 0, .5,
                     0, 0, -.5, 0, .5]).astype(np.float64)
        B.shape = (4, 4)
        for x in range(progress + 1):
            P = np.array([getTranslation(sixCow[(x+6-1) % 6]), getTranslation(sixCow[x]),
                         getTranslation(sixCow[(x+6+1) % 6]), getTranslation(sixCow[(x+6+2) % 6])])
            P.shape = (4, 3)
            interpolate[x] = A@B@P
        progress = 6
    elif progress == 6:
        ...
        t = animTime % 1.0
        point = np.array([t**3, t**2, t, 1]
                         )@interpolate[int((animTime % 6) // 1)]
        tangent = np.array([3*t*t, 2*t, 1, 0]
                           )@interpolate[int((animTime % 6) // 1)]
        up = np.array([0, 1, 0])
        vec_z = normalize(np.cross(tangent, up))
        vec_y = normalize(np.cross(vec_z, tangent))
        vec_x = normalize(tangent)
        cow2wld[0:3, 0] = vec_x
        cow2wld[0:3, 1] = vec_y
        cow2wld[0:3, 2] = vec_z
        ...
        if animTime // 6 == 3:
            progress = -1
            animStartTime = 0
            cow2wld[0:3, 0] = normalize(np.cross(up, vec_z))
            cow2wld[0:3, 1] = up
            cow2wld[0:3, 2] = vec_z

        setTranslation(cow2wld, point)
...
def initialize(window):
    global cursorOnCowBoundingBox, floorTexID, cameraIndex, camModel, cow2wld, cowModel, animStartTime
    cursorOnCowBoundingBox = False
    # Set up OpenGL state
